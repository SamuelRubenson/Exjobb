function [ RPpos ] = getRPpos(signals, corrMat, target_volatility)
  ...
  for t = 2:T
    ...
    for lambda = [1000000 100 10 2]
      ...
      mu = sum(abs(norm_mod_signal))/target_volatility^2/2;
      w_t = rpADMM(w0, adjusted_corrMat, mu, norm_mod_signal);
      ...
      %if any(abs(diff(w_t.*(adjusted_corrMat*w_t)./(abs(norm_mod_signal))))>0.01), disp('not mcr'); disp(w_t.*(adjusted_corrMat*w_t)./(abs(signal)/max(abs(signal)))); end
