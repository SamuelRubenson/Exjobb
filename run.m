...
LES_Params = struct('lookBack', 290, 'beta', -0.5, 'lambda', [0.3, 0.7, 1]);
LES = {'LES', LES_Params};
...
outCome = evaluatePerformance(Open(start:end,X), High(start:end,X), Low(start:end,X), Close(start:end,X), Config, assetClasses(X), TF{:}, MV{:}, RP{:}, RPM{:}, LES{:});
